...
class OpenAISession:
    def __init__(
        self,
        api_key: str,
        model: str = "gpt-4o-mini",   # 根据需要替换   :contentReference[oaicite:1]{index=1}
        ...
        self.client = openai.OpenAI(api_key=api_key, base_url="https://api.deepseek.com",
                                            http_client=httpx_client, timeout=timeout)
        # SDK 会自动带 Authorization :contentReference[oaicite:2]{index=2}
        self.model = model
    ...
    def send(
        ...
        *,
        ...
        """流式模式：回答 -> on_resp，思考链 -> on_think；两者均推送给 on_chunk"""
        self.history.append({"role": "user", "content": user_input})

        # 深拷贝 history 用于 payload 记录
        ...
        request_kwargs = {
            ...
            # 使用 stream_options 让官方 API 在流式结束时返回 usage  :contentReference[oaicite:3]{index=3}
            "stream_options": {"include_usage": True},
            **self.extra,
        }
        # 若用户希望显式传递“思考链”参数，可透传
        if self.enable_thinking:
            request_kwargs["enable_thinking"] = True
            request_kwargs["thinking_budget"] = self.thinking_budget

        # 调试：写入完整 payload
        fname = f"debug_payloads/payload_{self.model}_{int(time.time()*1000)}.json"
        Path(fname).write_text(json.dumps(request_kwargs, ensure_ascii=False, indent=2), encoding="utf-8")

        # ---- 调用官方 SDK（同步流式） ----
        stream_iter = self.client.chat.completions.create(**request_kwargs)   # ↳ 返回迭代器 :contentReference[oaicite:4]{index=4}

        usage = {"prompt_tokens": 0, "completion_tokens": 0, "total_tokens": 0}
        answer_parts: List[str] = []

        for chunk in stream_iter:
            # SDK 中每个 chunk 对象等价于 JSON (choices[0].delta.*)  :contentReference[oaicite:5]{index=5}
            choice_delta = chunk.choices[0].delta

            # 思考链（若模型实现）
            rc = getattr(choice_delta, "reasoning_content", None)        # :contentReference[oaicite:6]{index=6}
            if rc:
                if on_think:
                    on_think(rc)
                if on_chunk:
...
def main():
    ...
    model_analyst = "deepseek-reasoner"
