...
class SiliconFlowSession:
    ...
    def send(
        ...
        fname = f"debug_payloads/payload_{self.model.replace('/','_')}_{timestamp}.json"
        try:
            with open(fname, 'w', encoding='utf-8') as f:
                json.dump(payload, f, ensure_ascii=False, indent=2)
        except Exception as e:
            # 调试文件写入失败不影响主流程
            print(f"[WARN] 无法写入调试 payload: {e}")
        ...
        if stream:
            ...
            iterator = io.TextIOWrapper(resp.raw, encoding='utf-8', errors='ignore', newline='')
        ...
        # 将 assistant 回复加入 history
        assistant_msg = ''.join(full_reply)
...
def main():
    ...
    # 1. 需求分析师输出
    print("=== 需求分析师输出 ===")
    analyst.send(raw_req, on_chunk=stream_printer())
    analysis = analyst.history[-1]["content"]

    # 2. 开发者实现
    print("\n\n=== 开发者生成代码 ===")
    developer.send(f"开发需求如下：\n{analysis}", on_chunk=stream_printer())
